...
class GridParams:
    ...
    def __init__(
        ...
        self.grid_min_price_tick = grid_min_price_tick
    ...
    def get_level_price(self, side: OrderSide, level: int) -> Decimal:
        return self._round_to_grid_tick(self.mid_price - side.sign() * (self.grid_spread_in_quote / 2) * (level + 1) * (
    ...
    def _round_to_grid_tick(self, number: Decimal) -> Decimal:
        if self.grid_min_price_tick < 1:
            rounded = round(number / self.grid_min_price_tick) * self.grid_min_price_tick
        else:
            rounded = round(number * self.grid_min_price_tick) / self.grid_min_price_tick
