...
node_url = os.getenv("OP_GOERLI_NODE")
wallet = os.getenv("DEV_WALLET")
key = os.getenv("DEV_KEY")

# create network instance
network = Network.build(name=NetworkName.OPTIMISM_GOERLI, node_url=node_url)
...
read_only_weth = ERC20.from_network(name="WETH", network=network)
...
log.info(permissioned_market.offer(
    ...
    pay_gem=read_only_weth.address,
