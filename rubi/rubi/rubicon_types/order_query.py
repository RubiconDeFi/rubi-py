...
class OrderQuery:
    ...
    #####################################
    # General Helper Methods             # TODO: we will want to move this somewhere else most likely - we can't access the client from here though so we will need to figure out how to do that
    #####################################
    ...
    def offer_entity(
        ...
        if self.network: 
            ...
            Offer.pay_amt_formatted = SyntheticField(
                f=lambda pay_amt, pay_gem: self.network.get_token(pay_gem).to_decimal(pay_amt),
                type_=SyntheticField.FLOAT,
                deps=[Offer.pay_amt, Offer.pay_gem],
            )

            Offer.buy_amt_formatted = SyntheticField(
                f=lambda buy_amt, buy_gem: self.network.get_token(buy_gem).to_decimal(buy_amt),
                type_=SyntheticField.FLOAT,
                deps=[Offer.buy_amt, Offer.buy_gem],
            )

            Offer.paid_amt_formatted = SyntheticField(
                f=lambda paid_amt, pay_gem: self.network.get_token(pay_gem).to_decimal(paid_amt),
                type_=SyntheticField.FLOAT,
                deps=[Offer.paid_amt, Offer.pay_gem],
            )

            Offer.bought_amt_formatted = SyntheticField(
                f=lambda bought_amt, buy_gem: self.network.get_token(buy_gem).to_decimal(bought_amt),
                type_=SyntheticField.FLOAT,
                deps=[Offer.bought_amt, Offer.buy_gem],
            )

            Offer.pay_gem_symbol = SyntheticField(
                f=lambda pay_gem: self.network.get_token(pay_gem).symbol,
                type_=SyntheticField.STRING,
                deps=[Offer.pay_gem],
            )

            Offer.buy_gem_symbol = SyntheticField(
                f=lambda buy_gem: self.network.get_token(buy_gem).symbol,
                type_=SyntheticField.STRING,
                deps=[Offer.buy_gem],
            )
